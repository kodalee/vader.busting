...
//#endif //DEV
	...
	std::string text = XorStr("vader.tech ");
	...
	// background.
	Render::Engine::RectFilled(screen.x - size.m_width - 18, 10, size.m_width + 8, size.m_height + 8, Color(41, 32, 59, 255));

	// text.
	Render::Engine::segoe.string(screen.x - 14, 14, { 255, 255, 0, 255 }, text, Render::Engine::ALIGN_RIGHT);
...
void CEsp::SpectatorList() {
	std::vector< std::string > spectators{ XorStr("") };
	int h = Render::Engine::m_height;

	for (int i{ 1 }; i <= Interfaces::m_pGlobalVars->maxClients; ++i) {
		C_CSPlayer* player = reinterpret_cast<C_CSPlayer*>(Interfaces::m_pEntList->GetClientEntity(i));
		if (!player)
			continue;
		...
		if (player->m_hObserverTarget() != C_CSPlayer::GetLocalPlayer())
			continue;
	...
	for (size_t i{ }; i < spectators.size(); ++i) {
		...
		Render::Engine::segoe.string(screen.x - 15, (i * 14) - 10, { 255, 255, 255, 179 }, name, Render::Engine::ALIGN_RIGHT);
	}
}
...
void CEsp::Keybinds()
{
	...
	{
		Indicator_t ind{};
		ind.text = XorStr("override hitscan");
		ind.color = Color(0, 255, 0);
		indicators.push_back(ind);
	}
	...
	{
		Indicator_t ind{};
		ind.text = XorStr("override");
		ind.color = Color(0, 255, 0);
		indicators.push_back(ind);
	}

	if (g_Vars.misc.extended_backtrack_key.enabled)
	{
		Indicator_t ind{};
		ind.text = XorStr("ping spike [") + std::to_string(int(g_Vars.misc.extended_backtrack_time)) + XorStr("]");
		ind.color = Color(0, 255, 0);
		indicators.push_back(ind);
	}
	...
	{
		Indicator_t ind{};
		ind.text = XorStr("minimum damage");
		ind.color = Color(0, 255, 0);
		indicators.push_back(ind);
	}
	...
	Render::Engine::RectFilled(10, screen.y / 2 + 9, 150, 15, Color(41, 32, 59, 255));
	Render::Engine::esp.string(85, screen.y / 2 + 11, Color(255, 255, 0, 255), "keybinds", Render::Engine::ALIGN_CENTER);

	if (indicators.empty())
		return;

	Render::Engine::RectFilled(10, screen.x / 2 + 24, 150, (indicators.size() * 15) + 4, Color(0, 0, 0, 150));
	//render::rect(10, g_cl.m_height / 2 + 24, 150, (indicators.size() * 15) + 4, Color(40, 40, 40, 255)); // 300 is 64

	for (size_t i{ }; i < indicators.size(); ++i) {
		auto& indicator = indicators[i];
		auto size = Render::Engine::esp.size(indicator.text);

		Render::Engine::esp.string(13, (screen.y / 2 + 27) + (i * 15), Color::White(), indicator.text, Render::Engine::ALIGN_LEFT);
		Render::Engine::esp.string(140, (screen.y / 2 + 27) + (i * 15), indicator.color, XorStr("[active]"), Render::Engine::ALIGN_CENTER);
	}
}
