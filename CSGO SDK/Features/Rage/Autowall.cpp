...
void UTIL_TraceLine(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, C_CSPlayer* ignore, int collisionGroup, CGameTrace* ptr)
{
	Ray_t ray;
	ray.Init(vecAbsStart, vecAbsEnd);
	CTraceFilter traceFilter;
	traceFilter.pSkip = ignore;
	Interfaces::m_pEngineTrace->TraceRay(ray, mask, &traceFilter, ptr);
}
...
bool Autowall::CanHitFloatingPoint(const Vector& point, const Vector& source) {
	...
	static auto VectortoVectorVisible = [&](Vector src, Vector point) -> bool {
		...
		UTIL_TraceLine(src, point, MASK_SOLID, (C_CSPlayer*)local, 0, &TraceInit);
		CGameTrace Trace;
	...
	data.m_iPenetrationCount = 1;
	//data.trace_length = 0.0f;

	auto local_weapon = reinterpret_cast<C_WeaponCSBaseGun*>(local->m_hActiveWeapon().Get());
	if (!local_weapon)
		return false;

	auto weaponInfo = local_weapon->GetCSWeaponData();
	if (!weaponInfo.IsValid())
		return false;

	data.m_flCurrentDamage = (float)weaponInfo->m_iWeaponDamage;
