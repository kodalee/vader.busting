...
namespace ph_heartbeat {
	...
	*/
	...
	std::string PH_APPLICATION_ID = XorStr("6365c9b3996379dbabdbf38a1542e1a3");
	...
	__forceinline static void send_heartbeat(int task = PH_HEARTBEAT_TASK) {
		...
		switch (task) {
			case PH_HEARTBEAT_INIT_TASK: {
				...
				cJSON_AddItemToObject(heartbeat_json, XorStr("h1"), appid_hashed_json); // Xor the string "h1" for more protection

				cJSON* token_hashed_json = cJSON_CreateString(token_hashed.c_str());
				cJSON_AddItemToObject(heartbeat_json, XorStr("h2"), token_hashed_json); // Xor the string "h2" for more protection
			...
			case PH_HEARTBEAT_TASK: {
				...
				cJSON_AddItemToObject(heartbeat_json, XorStr("cipher"), cipher_json); // Xor the string "cipher" for more protection
				...
				cJSON_AddItemToObject(heartbeat_json, XorStr("iv"), iv_json); // Xor the string "iv" for more protection
			} break;
		}

		cJSON* task_json = cJSON_CreateNumber(task);
		cJSON_AddItemToObject(heartbeat_json, XorStr("task"), task_json); // Xor the string "task" for more protection
		...
		cJSON* cipher_value = cJSON_GetObjectItemCaseSensitive(json_return, XorStr("cipher")); // Xor the string "cipher" for more protection

		cJSON* iv_value = cJSON_GetObjectItemCaseSensitive(json_return, XorStr("iv")); // Xor the string "iv" for more protection
		...
		cJSON* success_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, XorStr("success")); // Xor the string "success" for more protection
		...
		cJSON* session_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, XorStr("session")); // Xor the string "session" for more protection
		...
		if (task == PH_HEARTBEAT_INIT_TASK) {
			...
			if (ph_heartbeat::username.empty()) {
				cJSON* user_value = cJSON_GetObjectItemCaseSensitive(cipher_ret_json, XorStr("user")); // Xor the string "user" for more protection
