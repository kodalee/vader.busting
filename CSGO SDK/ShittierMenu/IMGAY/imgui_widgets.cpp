...
bool ImGui::PlusButton(const char* label, bool type, const ImVec2& size_arg, const char* uniq_id, ImColor col, bool config)
{
    ...
    if (!config)
        PushFont(Menu::fonts.interfaces);
    if (!config)
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    window->DrawList->AddRectFilled(bb.Min, bb.Max, hovered ? GetColorU32(ImGuiCol_AccentDarker) : ImColor(80, 80, 80, 50), 2.f);
...
bool ImGui::TabEx(const char* icon, const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    if (selected || hovered) {
        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_AccentDarker),
            GetColorU32(ImGuiCol_AccentNoAlpha), GetColorU32(ImGuiCol_AccentNoAlpha), GetColorU32(ImGuiCol_AccentDarker));

        PopFont();
        PushFont(Menu::fonts.icons);
        window->DrawList->AddText(bb.Min + ImVec2(9.f, 8.f), GetColorU32(ImGuiCol_Text), icon);

        PopFont();
        PushFont(Menu::fonts.roboto);
        window->DrawList->AddText(bb.Min + ImVec2(40.f, 10.f), GetColorU32(ImGuiCol_Text), label);
    }
    else {
        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(80, 80, 80, 50),
            ImColor(80, 80, 80, 0), ImColor(80, 80, 80, 0), ImColor(80, 80, 80, 50));
...
bool ImGui::Tab(const char* icon, const char* label, const ImVec2& size_arg, bool selected)
{
    return TabEx(icon, label, size_arg, selected, ImGuiButtonFlags_None);
}

bool ImGui::SubTabEx(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    if (selected) {
        window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, (bb.Max.y - 30.f)), bb.Max, GetColorU32(ImGuiCol_AccentNoAlpha),
            GetColorU32(ImGuiCol_AccentNoAlpha), GetColorU32(ImGuiCol_AccentLight), GetColorU32(ImGuiCol_AccentLight));
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, (bb.Max.y - 1.f)), bb.Max, GetColorU32(ImGuiCol_AccentDarker));
        ...
        if (hovered) {
            ...
            window->DrawList->AddRectFilled(ImVec2(bb.Min.x, (bb.Max.y - 1.f)), bb.Max, ImColor(107, 107, 107));
...
bool ImGui::SubTab(const char* label, const ImVec2& size_arg, bool selected)
{
    return SubTabEx(label, size_arg, selected, ImGuiButtonFlags_None);
}

bool ImGui::DisabledTabEx(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_TextDisabled));
    RenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, ImVec2(0.2f, 0.3f), &bb);
    PopStyleColor();
...
bool ImGui::DisabledTab(const char* label, const ImVec2& size_arg, bool selected)
{
    return DisabledTabEx(label, size_arg, selected, ImGuiButtonFlags_None);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    window->DrawList->AddRectFilled(bb_frame.Min + ImVec2(-8.f, 3.f), bb_frame.Max - ImVec2(8.f, 10.f), ImColor(23, 23, 23));
    ...
    window->DrawList->AddRectFilled(grab_rect.Min + ImVec2(-8.f, 0.f), grab_rect.Max - ImVec2(8.f, 7.f), GetColorU32(ImGuiCol_Accent));
...
bool activateCheckboxAnimation = true;
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect frame_bb(pos, pos + ImVec2(14.f, 14.f));
    const ImRect total_bb(pos, pos + ImVec2(20.f + label_size.x, 14.f));
    ...
    // render
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, hovered ? GetColorU32(ImGuiCol_AccentDarker) : ImColor(80, 80, 80, 50), 2.f);

    if (*v) {
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_Accent), 2.f);
        //RenderCheckMark(window->DrawList, frame_bb.Min + ImVec2(2.f, 2.f), ImColor(23, 23, 23), 10.f);
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + 8.f, frame_bb.Min.y - 1.f), label);
...
bool ImGui::Keybind(const char* label, int* currentKey, int* keyType) {
    ImGuiContext& g = *GImGui;
    ImGuiIO* io = &GetIO();
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (*currentKey != 0 && g.ActiveId != id)
        strcpy_s(buf_display, keys[*currentKey]);
    else if (g.ActiveId == id)
        strcpy_s(buf_display, "Press a key");

    const float size = CalcTextSize(buf_display).x + 6.f;

    const ImRect rect(window->DC.CursorPos - ImVec2(size, 0), window->DC.CursorPos + ImVec2(0, 14));
    const ImRect clickable(window->DC.CursorPos - ImVec2(size, 0), window->DC.CursorPos + ImVec2(0, 14));
    ...
    auto IsPopupOpen = [](ImGuiID id) -> bool {
        ImGuiContext& g = *GImGui;
    ...
    if (g.ActiveId == id) {
        ...
        if (keyType) {
            ...
            if (popup_open) {
                SetNextWindowSizeConstraints(ImVec2(72.f, 0.0f), ImVec2(72.f, CalcMaxPopupHeightFromItemCount(12)));
                ...
                if (ImGuiWindow* popup_window = FindWindowByName(name))
                    if (popup_window->WasActive)
                    {
                        ...
                        ImVec2 pos = FindBestWindowPosForPopupEx(clickable.GetBL(), size_expected, &popup_window->AutoPosLastDirection,
                            r_outer, clickable, ImGuiPopupPositionPolicy_ComboBox);

                        SetNextWindowPos(pos + ImVec2(0.f, 2.f));
                    }
                ...
                PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0.f, style.WindowPadding.y + 5.f));
                PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0.f, 6.f));
                bool ret = BeginComboWindow(name, NULL, window_flags);

                auto comboWindow = GetCurrentWindow();
                //PostProcessing::performFullscreenBlur(comboWindow->DrawList, 1.f);
                comboWindow->DrawList->AddRectFilled(comboWindow->Pos - ImVec2(1.f, 1.f),
                    comboWindow->Pos + comboWindow->Size + ImVec2(1.f, 1.f), ImColor(30, 30, 30, 220), 2.f);

                if (Selectable("Always on", *keyType == 0))
                    *keyType = 0;

                if (Selectable("On key", *keyType == 1))
                    *keyType = 1;

                if (Selectable("Toggle", *keyType == 2))
                    *keyType = 2;

                if (Selectable("Off key", *keyType == 3))
                    *keyType = 3;
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, hovered ? GetColorU32(ImGuiCol_AccentDarker) : ImColor(80, 80, 80, 50), 2.f);
    RenderTextClipped(clickable.Min, clickable.Max, buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DC.CursorPos.y = window->DC.CursorPos.y + 15.f;
    ...
    const float w = 213.f;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 20.f));
    const ImVec2 pos = window->DC.CursorPos;
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddText(frame_bb.Min + ImVec2(6.f, 3.f), GetColorU32(ImGuiCol_Text), preview_value);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(frame_bb.Min + ImVec2(1.f, -16.f), GetColorU32(ImGuiCol_Text), label);
    ...
    else
    {
        ...
        if (flags & ImGuiComboFlags_HeightRegular)     popup_max_height_in_items = 12;
        else if (flags & ImGuiComboFlags_HeightSmall)  popup_max_height_in_items = 4;
        else if (flags & ImGuiComboFlags_HeightLarge)  popup_max_height_in_items = 20;
        SetNextWindowSizeConstraints(ImVec2(213.f, 0.0f), ImVec2(213.f, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    //PostProcessing::performFullscreenBlur(comboWindow->DrawList, 1.f);
    comboWindow->DrawList->AddRectFilled(comboWindow->Pos - ImVec2(1.f, 1.f),
        comboWindow->Pos + comboWindow->Size + ImVec2(1.f, 1.f), ImColor(30, 30, 30, 220), 3.f);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos - ImVec2(0.f, 13.f), pos + ImVec2(478.f, label_size.y + 19.f));
    ...
    auto FastFloatLerp = [&label](bool state, float min, float max, float speed) -> float {
        static std::unordered_map<const char*, float> valuesMapFloat;
        auto value = valuesMapFloat.find(label);

        if (value == valuesMapFloat.end()) {
            valuesMapFloat.insert({ label, min });
            value = valuesMapFloat.find(label);
        }
    ...
    float increase = FastFloatLerp(hovered, 0.f, 1.f, GetIO().DeltaTime * 5);

    //if (hovered && g.HoveredIdTimer < 0.001f) // yaaaassss
    //    Interfaces::surface->PlaySoundFromFilePath("UI/panorama/itemtile_rollover_07.wav");

    // render
    if (hovered)
        window->DrawList->AddRectFilled(total_bb.Min, ImVec2((total_bb.Min.x + (increase * 478.f)), total_bb.Max.y), ImColor(29, 29, 29), 3.f);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x + style.ItemInnerSpacing.x + 8.f, total_bb.Min.y + style.FramePadding.y + 13.f), label);
    ...
    //if (value_changed)
    //    Interfaces::surface->PlaySoundFromFilePath("UI/panorama/sidemenu_rollover_02.wav");

    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(280.f, 5.f), total_bb.Min + ImVec2(465.f, 48.f), ImColor(32, 32, 32), 3.f);
    window->DrawList->AddRect(total_bb.Min + ImVec2(280.f, 5.f), total_bb.Min + ImVec2(465.f, 48.f), ImColor(44, 44, 44), 3.f);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(total_bb.Min + ImVec2(280.f, 5.f), total_bb.Min + ImVec2(465.f, 48.f), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DC.CursorPos.y = window->DC.CursorPos.y + 13.f;
    ...
    const float w = 213.f;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 12.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max);
    ...
    if (grab_bb.Max.x > frame_bb.Min.x + 2.f)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0.f, 4.f), grab_bb.Max - ImVec2(0.f, 2.f), GetColorU32(ImGuiCol_Accent), 2.f);

    window->DrawList->AddRectFilled(ImVec2(grab_bb.Max.x - 2.f, grab_bb.Max.y - 8.f), ImVec2(grab_bb.Max.x + 2.f, grab_bb.Max.y), GetColorU32(ImGuiCol_Accent), 2.f);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(frame_bb.Min - ImVec2(0.f, 14.f), frame_bb.Max - ImVec2(1.f, 0.f), value_buf, value_buf_end, NULL, ImVec2(1.f, 0.05f));

    if (label_size.x > 0.0f)
        RenderText(frame_bb.Min + ImVec2(1.f, -14.f), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f - 1.f); // Arbitrary default of 8 lines high for multi-line
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding + ImVec2(2.f, 0.f);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x - 3, pos.y); // BUTTON POS - EXON
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (ImGui::Button("Copy", ImVec2(115 * GetStyle().ssadad, 20))) {
        memcpy(color_buffer, col, components * sizeof(float));
    }
    ImGui::SameLine(-1, 130 * GetStyle().ssadad);
    if (ImGui::Button("Paste", ImVec2(115 * GetStyle().ssadad, 20))) {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Render
    if (selected) {
        PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Accent));
        RenderTextClipped(text_min + ImVec2(6.f, -1.f), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
        PopStyleColor();
    }
    else {
        if (hovered) {
            PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_AccentDarker));
            RenderTextClipped(text_min + ImVec2(6.f, -1.f), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
            PopStyleColor();
        }
        else
